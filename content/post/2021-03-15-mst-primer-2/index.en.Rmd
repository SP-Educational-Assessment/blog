...
In the previous post on [previous post](/2021/03/08/ms-primer-1/) we considered some of the design considerations you need to think about when developing a new **multistage test** (MST).

In this post we'll describe the steps required for the development an MST. In particular we'll look at initial validation of a test design. This post contains some examples of how to perform this validation using R. 
...
Assuming the 1-2-4 pattern described in the [previous post](/2021/03/08/ms-primer-1/), with a total of 30 items (12 + 8 + 10), a simulation for the test design can be created from the following elements:

* a simulated item bank
* the modules and the items they contain
* the routing rules in the transition matrix

Average difficulty levels for each module are shown in Figure 1 below (where $\bar{b}$ is the mean difficulty of the module, and a standard deviation of 1 is assumed)

![Figure 1: MST Design with average difficulty levels](images/module-items.png)
...
```{r show simulated test, echo=TRUE, fig.cap="Example routing", message=FALSE}
plot(dfMSTResponse)
```
...
Both these graphs show that there is no variation between the different designs - they all show a similar pattern, with overestimation for higher ability levels. At this stage we may want to vary other aspects of the design (for example increasing the number of items, varying the cutoffs used for routing, or modifying the module difficulties). This process can be iterated on until we are happy that there are no further improvements we can make.
